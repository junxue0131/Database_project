...
public class TokenAuthenticationService {
    static final long EXPIRATIONTIME = 86_000_000;     // 1天
    ...
    static void addAuthentication(HttpServletResponse response, Authentication user) {
        ...
        try {
            ...
            Object[] arr = user.getAuthorities().toArray();
            JSONObject res = new JSONObject();
            res.put("userId", user.getName());
            res.put("token", JWT);
            res.put("userRole", arr[0]);
            response.getOutputStream().println(res.toJSONString());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
